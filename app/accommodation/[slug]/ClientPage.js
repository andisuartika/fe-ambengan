...
export default function ClientPage({ params }) {
  ...
  useEffect(() => {
    const fetchHomestay = async () => {
      try {
        ...
        console.error(err);
        setError("Failed to load homestay");
      }
    };
  ...
  const getRandomGalleryImages = (count = 5) => {
    const all =
      homestay.roomTypes?.flatMap((rt) => rt.galleries?.map((g) => g.url)) ||
      [];
    return all.sort(() => 0.5 - Math.random()).slice(0, count);
  };

  const galleryImages = getRandomGalleryImages();
